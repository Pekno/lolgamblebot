...
export class MainApi {
    ...
    constructor(apiHeader: RawAxiosRequestHeaders = {}, maxRequestsPerMinute: number = -1) {
    ...
    private async processQueue() {
        if (!this.queue?.length && (this.maxRequestsPerMinute !== -1 || this.currentRequestCount >= this.maxRequestsPerMinute))return;

        const { url, options, resolve, reject } = this.queue.shift()!;
        this.currentRequestCount++;

        try {
            Logger.debug(`${this.apiName} : PROCESS from queue "${url}" -> ${this.currentRequestCount}/${this.maxRequestsPerMinute}`);
